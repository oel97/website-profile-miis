...
@section('title', 'Tambah Periode PPDB')
    ...
    <div class="mx-auto max-w-5xl space-y-6">
        <div class="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h1 class="mt-2 text-3xl font-bold text-slate-900">Tambah Periode PPDB</h1>
                <p class="mt-2 text-sm text-slate-500">Buat informasi penerimaan peserta didik baru untuk tahun ajaran tertentu.</p>
            </div>
        ...
        <form action="{{ route('admin.ppdb-periods.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <input id="academic_year" name="academic_year" type="text" value="{{ old('academic_year') }}" required autofocus class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" placeholder="Contoh: 2026/2027">
                        @error('academic_year')
                    ...
                    <div>
                        ...
                        <input id="title" name="title" type="text" value="{{ old('title') }}" required class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" placeholder="Contoh: PPDB Gelombang 1">
                        @error('title')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input id="slug" name="slug" type="text" value="{{ old('slug') }}" class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" placeholder="Dibuat otomatis dari nama periode jika dikosongkan">
                        @error('slug')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input id="registration_start_at" name="registration_start_at" type="datetime-local" value="{{ old('registration_start_at') }}" required class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                    ...
                    <div>
                        ...
                        <input id="registration_end_at" name="registration_end_at" type="datetime-local" value="{{ old('registration_end_at') }}" required class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                    ...
                    <div>
                        ...
                        <input id="announcement_at" name="announcement_at" type="datetime-local" value="{{ old('announcement_at') }}" class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                    ...
                    <div>
                        ...
                        <input id="contact_whatsapp" name="contact_whatsapp" type="text" value="{{ old('contact_whatsapp') }}" class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" placeholder="Contoh: 081234567890">
                ...
                <div class="mt-5">
                    ...
                    <textarea id="description" name="description" rows="7" class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm leading-6 shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" placeholder="Jelaskan informasi penting periode PPDB ini.">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <label for="banner" class="mb-2 block text-sm font-semibold text-slate-700">Banner PPDB <span class="font-normal text-slate-400">(opsional)</span></label>
                ...
                @error('banner')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
                <img id="banner-preview" src="" alt="Preview banner PPDB" class="mt-4 hidden h-56 w-full rounded-xl object-cover ring-1 ring-slate-200">
            </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <label class="flex cursor-pointer items-center gap-3">
                        ...
                        <span>
                            ...
                            <span class="block text-xs text-slate-500">Periode yang ditampilkan akan tersedia pada halaman PPDB publik nantinya.</span>
                        </span>
                    </label>
                    @error('is_published')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                <button type="submit" class="rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700">Simpan Periode</button>
            </div>
        </form>
    </div>
