    ...
    <div class="mx-auto max-w-5xl space-y-6">
        <div class="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h1 class="mt-2 text-3xl font-bold text-slate-900">Tambah Prestasi</h1>
                <p class="mt-2 text-sm text-slate-500">Catat prestasi siswa, guru, atau sekolah.</p>
            </div>
        ...
        <form action="{{ route('admin.achievements.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="grid gap-5 md:grid-cols-2">
                    <div class="md:col-span-2">
                        ...
                        <input id="title" name="title" type="text" value="{{ old('title') }}" required autofocus class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" placeholder="Contoh: Juara 1 Olimpiade Matematika Tingkat Kabupaten">
                        @error('title')
                    ...
                    <div>
                        ...
                        <input id="achievement_date" name="achievement_date" type="date" value="{{ old('achievement_date') }}" class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                        <p class="mt-1 text-xs text-slate-500">Gunakan tanggal ini untuk mencatat tahun prestasi.</p>
                    ...
                    <div>
                        ...
                        <input id="recipient_name" name="recipient_name" type="text" value="{{ old('recipient_name') }}" class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" placeholder="Nama siswa, tim, guru, atau sekolah">
                    ...
                    <div>
                        ...
                        <input id="level" name="level" type="text" value="{{ old('level') }}" class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" placeholder="Contoh: Kabupaten, Provinsi, Nasional">
                        @error('level')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input id="organizer" name="organizer" type="text" value="{{ old('organizer') }}" class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" placeholder="Nama penyelenggara lomba atau kegiatan">
                        @error('organizer')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-5">
                    ...
                    <textarea id="description" name="description" rows="8" required class="w-full rounded-xl border-slate-300 px-3 py-2.5 text-sm leading-6 shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" placeholder="Jelaskan pencapaian dan informasi penting tentang prestasi ini">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    @error('image')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                    <img id="image-preview" src="" alt="Preview foto prestasi" class="mt-4 hidden h-48 w-full rounded-xl object-cover ring-1 ring-slate-200">
                </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <p class="mt-2 text-xs text-slate-500">Format PDF, JPG, JPEG, PNG, atau WEBP. Maksimal 5 MB.</p>
                    @error('certificate')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <label class="flex cursor-pointer items-center gap-3">
                    ...
                    <span>
                        ...
                        <span class="block text-xs text-slate-500">Prestasi yang ditampilkan akan tersedia pada halaman publik nantinya.</span>
                    </span>
                </label>
                @error('is_published')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                <button type="submit" class="rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700">Simpan Prestasi</button>
            </div>
        </form>
    </div>
